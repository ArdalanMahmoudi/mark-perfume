...
const OrderDashboardTemplate = ({ orders }) => {
  return (
    <div className="bg-secondary-layout">
      {/* search */}
      <div className="bg-white rounded-lg p-5 flex justify-between gap-4 border border-grey220 flex-wrap">
        <div className="flex gap-1 items-center">
          <label htmlFor="search" className="text-nowrap">جستجو کنید:</label>
          <input
            type="text"
            id="search"
            className="bg-secondary border border-grey220 w-full rounded-xs outline-0 p-1 text-sm"
          />
        </div>
        <div className="flex gap-1 items-center justify-end w-full lg:w-fit">
          نمایش
          <select className="bg-grey220 text-sm">
